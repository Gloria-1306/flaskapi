...
#from flask import Flask, abort # 4.6. Handling errors  
#import pymysql
#import os
...
#def movie(movie_id): 
    #db_conn = pymysql.connect(host="localhost", user="root", password = 'Glojai29ans!', database="bechdel",
...
# 4.1 
#@app.route("/movies1/<int:movie_id>") # chaque route est unique, on ne peut pas réutiliser les mêmes noms (same pour les noms de fonctions)
#def movie_1(movie_id):
    #db_conn = pymysql.connect(host="localhost", user="root", password = 'Glojai29ans!', database="bechdel",
                              #cursorclass=pymysql.cursors.DictCursor)
    #with db_conn.cursor() as cursor:
       # cursor.execute("SELECT * FROM Movies WHERE movieId=%s", (movie_id, ))
...
#@app.route("/movies2/<int:movie_id>")
##def movie_2(movie_id):
    #db_conn = pymysql.connect(host="localhost", user="root", password = 'Glojai29ans!', database="bechdel",
                              #cursorclass=pymysql.cursors.DictCursor)
    #with db_conn.cursor() as cursor:
...
# 4.5 Some cleanup and formatting on 'movie'
    #with db_conn.cursor() as cursor:
        #cursor.execute("""
        #SELECT
        #M.movieId,
...
# 4.5 Some cleanup and formatting on 'people'
    #with db_conn.cursor() as cursor:
        #cursor.execute("""
        #SELECT
       #P.personId,
...
# we can create a small function to remove them:
...
# And in the last (people) part:
    ...
    #if not movie: # 4.6. Handling errors
...
@auth.required
def movie(movie_id):
    db_conn = pymysql.connect(host="localhost", user="root", password='Glojai29ans!', database="bechdel",cursorclass=pymysql.cursors.DictCursor)
